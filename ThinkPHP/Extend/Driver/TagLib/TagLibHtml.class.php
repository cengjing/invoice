    ...
    public function _editor($attr,$content) {
     ...
     //   $content    =   $tag['content'];
        ...
        switch(strtoupper($type)) {
        	case 'KINDEDITOR':
        		$parseStr   =	'<!-- 编辑器调用开始 --><script type="text/javascript" src="__ROOT__/Public/js/kindEditor/kindeditor-min.js"></script><textarea id="'.$id.'" name="'.$name.'" style="width:700px;height:300px;visibility:hidden;">'.$content.'</textarea> <script type="text/javascript"> KindEditor.ready(function(K) { editor = K.create(\'textarea[name="content"]\', {allowFileManager : true	}); });</script> <!-- 编辑器调用结束 -->';
        		break;
            case 'FCKEDITOR':
