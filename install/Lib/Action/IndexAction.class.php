...
class IndexAction extends Action 
{
    ...
    public function install()
    {
    	...
    	$config = $db_config;
    	$db_config['ADMIN_USERNAME'] = $_REQUEST['ADMIN_USERNAME'];
    	$db_config['ADMIN_NAME'] = $_REQUEST['ADMIN_NAME'];
    	$db_config['ADMIN_PWD'] = $_REQUEST['ADMIN_PWD'];
    	...
    	{
    		$return_rs['msg'] = "连接数据库失败:".mysql_error();
    		$return_rs['status'] = false;
    	}
    	...
    	if($return_rs['status'])
    	{
    		...
    		//开始执行安装脚本
    		$msg = $this->restore($this->getRealPath()."/install/install.sql",$db_config);
    ...
    private function restore($file,$db_config)
    {
    	...
    	$db = M('','','mysql://'.$db_config['DB_USER'].':'.$db_config['DB_PWD'].'@'.$db_config['DB_HOST'].':'.$db_config['DB_PORT'].'/'.$db_config['DB_NAME']);
    	...
    	$sql = "INSERT INTO `".$db_config['DB_PREFIX'].
    			"user` VALUES (2,'".$db_config['ADMIN_USERNAME'].
    			"',1,'".$db_config['ADMIN_USERNAME']."',".time().",'".md5($db_config['ADMIN_PWD']).
    			"','',1,'',1,1,0,0,1,'','管理员','127.0.0.1',3,0);";
    	$db->query($sql);
    	$sql = "INSERT INTO `".$db_config['DB_PREFIX'].	"user_online` VALUES (2,".time().");";
    	$db->query($sql);
    	return "";
    }
